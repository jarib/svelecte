...
<script>
  ...
  export const getSelection = (onlyValues) => {
    ...
    const _selection = selectedOptions.map((opt) =>
      onlyValues
  ...
  function createFetch(fetch) {
    ...
    if (initFetchOnly) {
      if (typeof fetch === "string" && fetch.indexOf("[parent]") !== -1)
        return null;
      isFetchingData = true;
      options = [];
      debouncedFetch(null);
  ...
  let selectedOptions =
    value !== null
      ...
      : [];
  let selectedKeys = selectedOptions.reduce((set, opt) => {
  ...
  $: availableItems = maxReached
    ...
    : filterList(
        flatItems,
        disableSifter ? null : $inputValue,
        multiple ? selectedKeys : false,
        searchField,
        sortField,
        itemConfig
      );
  ...
  $: itemRenderer =
    typeof renderer === "function"
      ? renderer
      : formatterList[renderer] ||
        formatterList.default.bind({ label: currentLabelField });
  $: {
  ...
  function handleValueUpdate(passedVal) {
    ...
    if (passedVal) {
      ...
      readSelection = Array.isArray(passedVal)
        ? _selection
        : _selection.shift();
    }
    prevValue = passedVal;
  ...
  function deselectOption(opt) {
    if (opt.$created && backspacePressed && allowEditing) {
      alreadyCreated.splice(
        alreadyCreated.findIndex(
          (o) => o === opt[labelAsValue ? currentLabelField : currentValueField]
        ),
        1
      );
  ...
  function onKeyDown(event) {
    ...
    const Tab =
      selectOnTab && $hasDropdownOpened && !event.shiftKey ? "Tab" : "No-tab";
    ...
    switch (event.key) {
      ...
      case "PageDown":
        if (isPageEvent) {
          ...
          dropdownActiveIndex = Math.ceil(
            (item * dropdownActiveIndex + wrap) / item
          );
        }
      case "ArrowUp":
        event.preventDefault();
      ...
      case "PageUp":
        if (isPageEvent) {
          const [wrap, item] = refDropdown.getDimensions();
          dropdownActiveIndex = Math.floor(
            (item * dropdownActiveIndex - wrap) / item
          );
        }
      case "ArrowDown":
      ...
      case "Enter":
        ...
        let activeDropdownItem = !ctrlKey
          ? availableItems[dropdownActiveIndex]
          : null;
        if (creatable && $inputValue) {
          activeDropdownItem =
            !activeDropdownItem || ctrlKey ? $inputValue : activeDropdownItem;
          ctrlKey = false;
        }
        !ctrlKey && activeDropdownItem && onSelect(null, activeDropdownItem);
        if (availableItems.length <= dropdownActiveIndex) {
          dropdownActiveIndex =
            currentListLength > 0 ? currentListLength : listIndex.first;
        }
      ...
      case "Delete":
        if ($inputValue === "" && selectedOptions.length) {
          ctrlKey
            ...
            : onDeselect(null, selectedOptions[selectedOptions.length - 1]);
          event.preventDefault();
        }
  ...
  onMount(() => {
    ...
    if (creatable) {
      const valueProp = itemConfig.labelAsValue
        ? currentLabelField
        : currentValueField;
      alreadyCreated = [""].concat(
        flatItems.map((opt) => opt[valueProp]).filter((opt) => opt)
      );
    }
    dropdownActiveIndex = listIndex.first;
    if (prevValue && !multiple) {
      ...
      dropdownActiveIndex = flatItems.findIndex(
        (opt) => opt[prop] === selectedProp
      );
    }
    isIOS = iOS();
    isAndroid = android();
...
<div class={`svelecte ${className}`} class:is-disabled={disabled} {style}>
  <Control
    ...
    collapseSelection={collapseSelection
      ? config.collapseSelectionFn.bind(_i18n)
      : null}
    {inputValue}
    {hasFocus}
    {hasDropdownOpened}
    ...
    on:blur
  >
    <div slot="icon" class="icon-slot"><slot name="icon" /></div>
  </Control>
  <Dropdown
  ...
  {#if name && !hasAnchor}
    <select
      ...
      {required}
      {disabled}
      use:refSelectAction={refSelectActionParams}
    >
      {#each selectedOptions as opt}
        <option
          value={opt[labelAsValue ? currentLabelField : currentValueField]}
          selected>{opt[currentLabelField]}</option
        >
      {/each}
    </select>
  {/if}
...
<style>
  ...
  :global(.svelecte-control .sv-item.is-disabled) {
    opacity: 0.5;
    cursor: not-allowed;
  }
